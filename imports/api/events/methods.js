 ...
 */
    export const getAggregatedGraph = new ValidatedMethod({
        ...
        validate: null,
        run({ from, to, limit }) {
            console.log('getting aggregated graph');
     ...
     */
    export const getNodeAncestorGraph = new ValidatedMethod({
        name: 'getNodeAncestorGraph',
        validate: null,
        run({ nodeId }) {
            let createAncestorGraph = function (graph, nodeId) {
                let event = Events.findOne({'meta.id': nodeId});
